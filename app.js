...
function doLogin() {
  ...
  if (pass === ADMIN_PASS) {
    ...
    setTimeout(pnInit, 800); // init push notifications after Firestore connects
  } else {
...
self.addEventListener('push', (event) => {
  let payload = { title: 'MiNutriQ Admin', body: 'New notification' };
  try { if (event.data) payload = event.data.json(); } catch(e) {}
  // FCM wraps payload under 'notification' key
  const n = payload.notification || payload;
  event.waitUntil(
    self.registration.showNotification(n.title || 'MiNutriQ', {
      body:    n.body    || '',
      icon:    n.icon    || './',
      badge:   n.badge   || './',
      tag:     n.tag     || 'minutriq-push',
      vibrate: [100, 50, 100],
      data:    { url: (payload.fcm_options && payload.fcm_options.link) || self.registration.scope },
    })
  );
});
...
/* ═══════════════════════════════════════════════════════════
   ...
   • Stores VAPID public key + subscriber list in Firestore
     collection: push_subscriptions  (doc per subscriber endpoint)
   • Admin subscribes this browser, views all subscribers,
     sends test/broadcast via Firestore-triggered Cloud Messaging
     OR records the payload for a backend worker to send.
   • Because the Admin Console is a pure client-side PWA, actual
     message delivery requires a server / Cloud Function that reads
     the Firestore push_subscriptions collection and calls the
     Web Push protocol. This module handles the client-side half:
     requesting permission, creating the PushSubscription object,
     storing it in Firestore, and letting the admin compose payloads.
═══════════════════════════════════════════════════════════ */

const PN_VAPID_KEY  = 'pn_vapid_public_key';   // localStorage key
const PN_COLLECTION = 'push_subscriptions';     // Firestore collection

/** In-memory list of subscriber docs fetched from Firestore */
let _pnSubs = [];

/** Current PushSubscription of this browser (if subscribed) */
let _pnThisSub = null;

/* ── helpers ── */
function _pnLog(msg, type = 'info') {
  const el = document.getElementById('pn-log');
  if (!el) return;
  const now = new Date().toLocaleTimeString('en-GB', { hour:'2-digit', minute:'2-digit', second:'2-digit' });
  const cls = `pn-log-${type}`;
  el.innerHTML += (el.innerHTML.trim() ? '<br>' : '') +
    `<span class="${cls}">[${now}] ${msg}</span>`;
  el.scrollTop = el.scrollHeight;
}

function _pnSetStatus(state, text) {
  const dot  = document.getElementById('pn-status-dot');
  const txt  = document.getElementById('pn-status-text');
  if (!dot || !txt) return;
  dot.className = 'pn-status-dot ' + (state || '');
  txt.textContent = text;
}

function _pnUpdateSubBadge(show) {
  const badge = document.getElementById('pn-sub-badge');
...
function pnInit() {
  // Restore saved VAPID key
  const saved = localStorage.getItem(PN_VAPID_KEY);
  if (saved) {
    const inp = document.getElementById('pn-vapid-input');
    if (inp) inp.value = saved;
  }

  // Check browser support
  if (!('serviceWorker' in navigator) || !('PushManager' in window)) {
    _pnSetStatus('err', 'Push Notifications not supported in this browser.');
    const btn = document.getElementById('pn-subscribe-btn');
  ...
  if (Notification.permission === 'granted') {
    _pnSetStatus('ok', 'Notifications permitted · checking subscription…');
    _pnCheckExistingSub();
  } else if (Notification.permission === 'denied') {
    _pnSetStatus('err', 'Notifications blocked — allow them in browser settings.');
    const btn = document.getElementById('pn-subscribe-btn');
    if (btn) btn.disabled = true;
  } else {
    _pnSetStatus('warn', 'Permission not yet granted — click "Subscribe This Browser".');
  }
...
async function _pnCheckExistingSub() {
  try {
    const reg = await navigator.serviceWorker.ready;
    const sub = await reg.pushManager.getSubscription();
    if (sub) {
      _pnThisSub = sub;
      _pnSetStatus('ok', 'Subscribed ✓ · This browser will receive push notifications.');
      _pnUpdateSubBadge(true);
      const btn = document.getElementById('pn-test-btn');
      if (btn) btn.disabled = false;
    } else {
      _pnSetStatus('ok', 'Notifications permitted · not yet subscribed on this browser.');
    }
  } catch (e) {
    _pnSetStatus('warn', 'Could not check subscription: ' + e.message);
  }
}

/* ── Save VAPID key ── */
function pnSaveVapid() {
  const inp = document.getElementById('pn-vapid-input');
  const key = inp ? inp.value.trim() : '';
  if (!key) {
    _pnLog('⚠ Paste a VAPID public key first.', 'warn');
    return;
  }
  // Basic length sanity check (VAPID public keys are ~87 chars Base64url)
  if (key.length < 50) {
    _pnLog('✕ Key looks too short — paste the full Base64url VAPID public key.', 'err');
    return;
  }
  localStorage.setItem(PN_VAPID_KEY, key);
  _pnLog('✓ VAPID public key saved to local storage.', 'ok');
  showToast('VAPID key saved ✓');
}

/* ── Subscribe this browser ── */
async function pnSubscribeThis() {
  const btn = document.getElementById('pn-subscribe-btn');
  if (btn) btn.disabled = true;
  _pnSetStatus('warn', 'Requesting permission…');

  try {
    const permission = await Notification.requestPermission();
    if (permission !== 'granted') {
      _pnSetStatus('err', 'Permission denied — allow notifications in browser settings.');
      _pnLog('✕ User denied notification permission.', 'err');
      if (btn) btn.disabled = false;
      return;
    }

    _pnSetStatus('warn', 'Getting FCM token…');

    // Use Firebase Messaging to get FCM token
    const messaging = firebase.messaging();
    const fcmToken  = await messaging.getToken({
      vapidKey: 'BCRx8VpxOPxrLMGkSJOGZr-WJfQeHEEzXMCA7OyOiMEdE85Kw5L4kHlt2zDISl8BsKq-usibD3KA6f7MGE_s77g',
    });

    if (!fcmToken) throw new Error('FCM token was empty — check VAPID key');

    _pnThisSub = fcmToken;
    _pnSetStatus('ok', 'Subscribed ✓ · Saving to Firestore…');
    _pnUpdateSubBadge(true);

    const docId = fcmToken.substring(0, 28);
    const subData = {
      fcmToken,
    ...
    if (db) {
      await db.collection(PN_COLLECTION).doc(docId).set(subData);
      _pnLog('✓ FCM token saved to Firestore (' + PN_COLLECTION + '/' + docId + ').', 'ok');
    } else {
      _pnLog('⚠ Firestore offline — token stored locally only.', 'warn');
    }
    ...
    _pnSetStatus('err', 'Subscription failed: ' + e.message);
    _pnLog('✕ Error: ' + e.message, 'err');
  }

  if (btn) btn.disabled = false;
}

/* ── Test notification (local, shown by SW) ── */
async function pnTestNotif() {
  if (Notification.permission !== 'granted') {
    _pnLog('✕ Permission not granted.', 'err'); return;
  }
  try {
    const reg = await navigator.serviceWorker.ready;
    reg.showNotification('MiNutriQ Admin — Test ✓', {
    ...
    _pnLog('✓ Test notification dispatched via Service Worker.', 'ok');
  } catch (e) {
    _pnLog('✕ showNotification error: ' + e.message, 'err');
  }
}

/* ── Load subscribers from Firestore ── */
async function pnLoadSubscribers() {
  if (!db) {
    _pnRenderSubs([]);
    _pnLog('⚠ Firestore not connected — cannot load subscribers.', 'warn');
    return;
  }
  try {
    const snap = await db.collection(PN_COLLECTION).orderBy('subscribedAt', 'desc').get();
    _pnSubs = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    _pnRenderSubs(_pnSubs);
  } catch (e) {
    _pnLog('⚠ Could not load subscribers: ' + e.message, 'warn');
  }
}

function _pnRenderSubs(subs) {
  const tbody = document.getElementById('pn-subs-tbody');
  const count = document.getElementById('pn-sub-count');
  if (!tbody) return;
  if (count) count.textContent = subs.length;

  if (!subs.length) {
    tbody.innerHTML = '<tr><td colspan="4"><div class="pn-empty">No subscribers yet</div></td></tr>';
    return;
  }

  tbody.innerHTML = subs.map(s => {
    const keyShort = s.keys?.p256dh
      ? s.keys.p256dh.substring(0, 8) + '…' + s.keys.p256dh.slice(-4)
      : (s.id.substring(0, 10) + '…');
    const date = s.subscribedAt
      ? new Date(s.subscribedAt).toLocaleDateString('en-GB', { day:'2-digit', month:'short', year:'2-digit' })
      : '—';
    const statusClass = s.status === 'active' ? 'active' : s.status === 'error' ? 'error' : 'pending';
    const statusTxt   = s.status === 'active' ? '● Active' : s.status === 'error' ? '✕ Error' : '◌ Pending';
    return `<tr>
      <td><span class="pn-key-short">${keyShort}</span></td>
      <td>${date}</td>
      <td><span class="pn-status-pill ${statusClass}">${statusTxt}</span></td>
      <td>
        <button class="pn-del-btn" onclick="pnDeleteSub('${s.id}')">🗑 Remove</button>
      </td>
    </tr>`;
  }).join('');

  // Enable broadcast if there are active subscribers
  const bcastBtn = document.getElementById('pn-bcast-btn');
  if (bcastBtn) bcastBtn.disabled = subs.filter(s => s.status === 'active').length === 0;
}

/* ── Delete a subscriber ── */
async function pnDeleteSub(docId) {
  if (!db) { _pnLog('✕ Firestore not connected.', 'err'); return; }
  if (!confirm('Remove this subscriber?')) return;
  try {
    await db.collection(PN_COLLECTION).doc(docId).delete();
    _pnLog(`✓ Subscriber ${docId.substring(0,8)}… removed.`, 'ok');
    pnLoadSubscribers();
    showToast('Subscriber removed');
  } catch (e) {
    _pnLog('✕ Delete failed: ' + e.message, 'err');
  }
}

/* ── Broadcast payload to all subscribers via Firestore ── */
/* ══════════════════════════════════════════════════════════
   FCM HTTP v1 — direct push via Google OAuth2 service account
   No Cloudflare, no Cloud Functions, no terminal needed.
══════════════════════════════════════════════════════════ */
const _FCM_PROJECT_ID    = 'nutri-track-pro-c11c5';
const _FCM_CLIENT_EMAIL  = 'firebase-adminsdk-fbsvc@nutri-track-pro-c11c5.iam.gserviceaccount.com';
const _FCM_PRIVATE_KEY   = `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`;

/* ── Get a short-lived OAuth2 access token from Google ── */
async function _fcmGetAccessToken() {
  const now     = Math.floor(Date.now() / 1000);
  const header  = { alg: 'RS256', typ: 'JWT' };
  const payload = {
    iss:   _FCM_CLIENT_EMAIL,
  ...
  const pemBody = _FCM_PRIVATE_KEY
...
/* ── Send one FCM message to one subscriber token ── */
async function _fcmSendOne(accessToken, fcmToken, title, body) {
  const res = await fetch(
    `https://fcm.googleapis.com/v1/projects/${_FCM_PROJECT_ID}/messages:send`,
    {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        message: {
          ...
          webpush: {
            ...
            fcm_options: { link: window.location.href },
          },
        },
      }),
    }
...
async function pnBroadcast() {
  const title = document.getElementById('pn-bcast-title')?.value.trim();
  const body  = document.getElementById('pn-bcast-body')?.value.trim();
  const btn   = document.getElementById('pn-bcast-btn');

  if (!title) { _pnLog('⚠ Enter a notification title.', 'warn'); return; }
  if (!body)  { _pnLog('⚠ Enter a message body.', 'warn');  return; }

  const activeSubs = _pnSubs.filter(s => s.status === 'active' && s.fcmToken);
  if (!activeSubs.length) {
    _pnLog('⚠ No FCM-registered subscribers yet. Ask users to subscribe via the MiNutriQ PWA.', 'warn');
    return;
  }

  if (btn) btn.disabled = true;
  _pnLog(`⏳ Getting access token…`, 'info');

  try {
    const accessToken = await _fcmGetAccessToken();
    _pnLog(`✓ Authenticated with FCM · sending to ${activeSubs.length} subscriber(s)…`, 'ok');

    const results = await Promise.allSettled(
      activeSubs.map(s => _fcmSendOne(accessToken, s.fcmToken, title, body))
    );

    const sent   = results.filter(r => r.status === 'fulfilled' && r.value).length;
    const failed = results.length - sent;

    _pnLog(`✓ Done! ${sent}/${activeSubs.length} delivered · ${failed} failed.`, 'ok');

    // Local preview for admin
    if (Notification.permission === 'granted') {
      const reg = await navigator.serviceWorker.ready;
      reg.showNotification(title, { body, tag: 'pn-broadcast-preview', icon: './favicon.png' });
    }

    showToast(`Delivered to ${sent} subscriber(s) ✓`);
    document.getElementById('pn-bcast-title').value = '';
    document.getElementById('pn-bcast-body').value  = '';

    // Log to Firestore
    if (db) {
      db.collection('push_broadcasts').add({
        title, body, sentAt: new Date().toISOString(),
        sent, failed, total: activeSubs.length,
      }).catch(() => {});
    }

  } catch (e) {
    _pnLog('✕ Broadcast failed: ' + e.message, 'err');
    showToast('Broadcast failed — check log');
  }
